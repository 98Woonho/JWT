...
public class CustomLogoutSuccessHandler implements LogoutSuccessHandler {
    ...
    @Value("${spring.security.oauth2.client.registration.kakao.client-id}") // [공부] @Value : application.properties의 property 값을 필드에 주입해주는 Annotation. @Value를 사용할려면 @Value를 사용하고 있는 메서드의 클래스가 Bean으로 등록이 되어 있어야 함.
    ...
    public void onLogoutSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        System.out.println("[CustomLogoutSuccessHandler] logout() provider : " + provider);
        ...
        if (provider != null && provider.equals("kakao")) {
            ...
            // "https://nid.naver.com/nidlogin.logout?returl=http://localhost:8080/" 을 해도 네이버로 이동하는 이유는, 네이버 측에서 returl에 네이버 관련 링크가 아니면 naver.com로 redirect되도록 설정을 해놓았기 때문이다.=
